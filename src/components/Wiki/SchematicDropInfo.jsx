...
const SchematicDropInfo = ({ name, items }) => {
  ...
  const showSchematics = (schematics) => {
    return schematics.map((schematic) => {
      ...
      return (
        <li className="inline-block mr-2 mb-2" key={`schematic-${schematic.name}`}>
          <div className="p-2 bg-gray-800 border border-gray-700 rounded-lg flex items-center space-x-2">
            ...
            <a href={url} className="text-blue-400 hover:text-blue-300">{t(schematic.name, { ns: "items" })}</a>
          </div>
        </li>
      );
    });
      ...
      return (
        <div className="w-full md:w-1/2 p-4">
          <div className="bg-gray-800 border border-gray-700 rounded-lg overflow-hidden">
            <div className="p-3 bg-gray-900 border-b border-gray-700 text-neutral-300">{t("Learned in")}</div>
            <div className="p-4">
              <ul className="flex flex-wrap -m-2">{showSchematics(schematics)}</ul>
            </div>
          </div>
        </div>
      );

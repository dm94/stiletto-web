...
const WikiContent = ({
  ...
  const content = useMemo(() => {
    if (isLoading) {
      return (
        <section className="w-full" key="wiki-loading" aria-live="polite" aria-busy="true">
    ...
    if (contentType === "creatures" && displayedCreatures.length > 0) {
      return displayedCreatures.map((creature, index) => (
        ...
        >
          <Link to={getCreatureUrl(creature.name)} aria-label={t(creature.name, { ns: "creatures" })}>
            <div className="bg-gray-800 border border-gray-700 hover:border-blue-500 rounded-lg overflow-hidden shadow-md transition-all duration-300 hover:shadow-lg hover:transform hover:scale-102">
  ...
  return <section className="flex flex-wrap -m-3" aria-label={t(contentType === "items" ? "wiki.items" : "wiki.creatures")}>{content}</section>;
};
